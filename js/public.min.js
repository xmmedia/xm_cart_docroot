...
cart_public_app.models.order_product=Backbone.Model.extend({parse:function(resp){return _.each(["id","cart_product_id","quantity"],function(field){void 0!==typeof resp[field]&&(resp[field]=parseInt(resp[field],10))}),resp.unit_price=parseFloat(resp.unit_price),resp.amount=parseFloat(resp.amount),resp}});

cart_public_app.models.total=Backbone.Model.extend({parse:function(resp){return _.each(["value"],function(field){void 0!==typeof resp[field]&&(resp[field]=parseFloat(resp[field]))}),resp}});
